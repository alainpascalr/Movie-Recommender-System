				...
				<div class="col-md-12 column">
					<form id="formID" name="formID" method="post" action="" role="form">
						<div class="row">
							<div class="form-group-xs">
								 ...
								 <input name ="input-email" type="email" class="form-control" id="input-email"  placeholder="abcde@hotmail.com" required autofocus/>
							</div>
						</div>
						<div class="row">
							<div class="form-group-xs">
					...
					<?php
						if (array_key_exists('input-email', $_POST) && array_key_exists('input-pw', $_POST)){
							...
							$result = pg_query("SELECT * FROM users WHERE users.email = '$getEmail';") or die('Query failed: ' . pg_last_error());
							
							$numRows = pg_num_rows($result);
							
							if($numRows != 0){
								$res = pg_query("SELECT * FROM users WHERE users.email = '$getEmail';") or die('Query failed: ' . pg_last_error());
